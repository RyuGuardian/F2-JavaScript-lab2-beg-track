...
--------------------|------------------|------------------|
 ...
 with Dowington. */
function Blob(hunger) {
  this.initialHunger = hunger;  //starting rate in people per hour
}
...
var blobHunger = blob.initialHunger;

while (dowingtonPop > 0) {
  dowingtonPop -= blobHunger;
  blobHunger++;
  hoursInDowington++;
}
...
hoursSpentInDowington = hoursInDowington;
...
function variablify(string) {
  ...
  var varString = strArray[0];

  for (var i = 1; i < strArray.length; i++) {
    varString += strArray[i].charAt(0).toUpperCase() + strArray[i].substring(1);
  }
...
assert(variablify('Bibbidy Bobbidy Buu') === 'bibbidyBobbidyBuu',
  'variablify(\'Bibbidy Bobbidy Buu\')');
assert(variablify('Excelsior!') === 'excelsior',
  'variablify(\'Excelsior!\')');
assert(variablify('Asterisk*, dollar$, semicolon;.') === 'asteriskDollarSemicolon',
  'variablify(\'Asterisk*, dollar$, semicolon;.\')');
